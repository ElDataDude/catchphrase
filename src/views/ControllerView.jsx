...
const ControllerView = () => {
  const { state, dispatch, presence } = useQuiz();
  ...
  const openDisplayView = () => {
    window.open(`/quiz/${state.id}?view=display`, '_blank');
  };
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
          ...
          <div className="text-[11px] text-white/60 flex items-center gap-1">
            <span
              className={[
                ...
                presence?.display > 0 ? 'bg-green-400' : 'bg-white/20'
              ].join(' ')}
            />
            {presence?.display > 0 ? 'Display connected' : 'Display not open'}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        <div className="surface overflow-hidden" style={{ height: '170px' }}>
        ...
        <ControlPanel onResetQuestion={handleResetQuestion} />
